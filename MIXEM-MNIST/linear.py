...
def get_data_loaders(dsname,shuffle=False, cats='',pickle_path=None,**kwargs):
  ...
  if dsname == 'STL10':
    ...
    train_dataset = datasets.STL10('{}/{}'.format(data_root,dsname), split='train', download=True, transform=trans)
    test_dataset = datasets.STL10('{}/{}'.format(data_root,dsname), split='test', download=True, transform=trans)
  elif dsname == 'CIFAR10':
    train_dataset = datasets.CIFAR10('{}/{}'.format(data_root,dsname), train=True, download=True, transform=transforms.ToTensor())
    test_dataset = datasets.CIFAR10('{}/{}'.format(data_root,dsname), train=False, download=True, transform=transforms.ToTensor())
  elif dsname == 'CIFAR100':
    train_dataset = datasets.CIFAR100('{}/{}'.format(data_root,dsname), train=True, download=True, transform=transforms.ToTensor())
    test_dataset = datasets.CIFAR100('{}/{}'.format(data_root,dsname), train=False, download=True, transform=transforms.ToTensor())
  elif dsname == 'MNIST':
    trans = transforms.Compose([transforms.Grayscale(3),transforms.Resize((28,28)),transforms.ToTensor()])
    train_dataset = datasets.MNIST(root='{}/{}'.format(data_root,dsname), train=True, download=True, transform=trans)
    test_dataset = datasets.MNIST(root='{}/{}'.format(data_root,dsname), train=False, download=True, transform=trans)
...
def linear_main(lineardataroot,_device,model_dir,model_name,n_classes=None,pickle_path=None,model=None,**kwargs):
  ...
  if args.dsname:
    ...
    dsname = 'CIFAR100'
  elif args.model_name.startswith('MNIST'):
    dsname = 'MNIST'
