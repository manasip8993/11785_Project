...
if __name__ == '__main__':
    ...
    parser.add_argument('tsne_plot',type=str, default='output')

    args = parser.parse_args(sys.argv[1:])
    for k,v in sorted(vars(args).items()):
        print('{}: {}'.format(str(k).ljust(20),v ) )

    features, y_test = load_features(args.data_root,args.model_path, args.config_path, args.n_classes,args.dsname)
    X =  cluster(features, args.n_classes)
    print(X.dtype)
    print(y_test.shape)
    print(y_test)
    X = X.astype('float')
    # print(features)
    # print(len(features['features']['fc'][0]))
    # X = features['features']['fc'][0]
    ...
    #                 data=df).set(title="STL-10 data T-SNE projection")
    fig, ax = plt.subplots()

    colors = {0:'red', 1:'green', 2:'blue', 3:'yellow',\
              4:'pink', 5:'black', 6:'orange', 7:'brown',\
              8:'magenta', 9:'cyan'}


    ax.scatter(df["comp-1"], df["comp-2"], c=df["y"].map(colors), s=2)

    plt.show()
    fig.savefig('./tsne_plot-{}.png'.format(args.tsne_plot))
